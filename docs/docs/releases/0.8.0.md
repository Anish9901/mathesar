...
Mathesar 0.8.0 makes it easier to work with connected data by introducing **related columns**. You can now extend a table to show columns from related tables directly in the table page. This release also adds nested filtering for more complex queries and support for working with database views.
...
Use the "Extend" button to display columns from related tables into your current table view. After displaying the columns, you can edit them by adding or removing values. You'll see a dropdown of avaliable choices with checkboxes to add or remove them from the record.
